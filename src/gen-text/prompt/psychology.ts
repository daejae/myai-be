...
export const getLongPsychology = async (
  ...
  const draft = await openai.createChat({
    userPrompt: `인터넷에서 흔히 볼 수 있는 심리학에 대한 재미있는 썰을 매우 길게 작성해줘. title과 story로 JSON 객체로 반환해줘.`,
    model: 'gpt-4o',
    isJson: true,
  });
...
export const getShortPsychology = async (
  ...
  const draft = await openai.createChat({
    userPrompt: `인터넷에서 흔히 볼 수 있는 심리학에 대한 재미있는 썰을 작성해줘. title과 story로 JSON 객체로 반환해줘.`,
    model: 'gpt-4o',
    isJson: true,
  });
